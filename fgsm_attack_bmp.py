...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run a targeted FGSM attack on a BMP image for LLaVA."
    )
    parser.add_argument("--image", required=True, help="Input BMP image path.")
    parser.add_argument(
        "--output",
        default="fgsm_attacked.bmp",
    ...
    parser.add_argument(
        ...
        type=float,
        default=8.0 / 255.0,
        help="FGSM perturbation budget in image space. Default: 8/255.",
    )
    parser.add_argument(
        "--device",
...
def image_bounds(pixel_values: torch.Tensor, processor, eps: float):
...
def main():
    ...
    adv_pixel_values = base_pixel_values.detach().clone().requires_grad_(True)
    outputs = model(
    ...
    adv_pixel_values = adv_pixel_values - eps_tensor * adv_pixel_values.grad.sign()
    adv_pixel_values = torch.max(torch.min(adv_pixel_values, base_pixel_values + eps_tensor), base_pixel_values - eps_tensor)
    ...
    print(f"Saved FGSM adversarial BMP to: {args.output}")
    print(f"Targeted loss: {loss.item():.6f}")
